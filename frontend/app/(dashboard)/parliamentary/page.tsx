    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {PRESETS.map((p, idx) => (
          <Card
            ...
            className="p-4 cursor-pointer hover:border-gold-500/50 hover:bg-navy-900/60 transition-all border-slate-800/80 group"
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-mono text-gold-400 font-bold uppercase tracking-wider">{p.type}</span>
              <ArrowRight className="h-3.5 w-3.5 text-slate-400 group-hover:text-gold-400 transition-colors" />
            </div>
            <h4 className="text-sm font-semibold text-slate-200 group-hover:text-slate-100 mb-1">{p.title}</h4>
            <p className="text-xs text-slate-400 line-clamp-2">{p.question}</p>
          </Card>
        ))}
      </div>
      ...
      <Card className="border-slate-800/90 shadow-card-dark">
        ...
        <CardContent className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-mono text-slate-300 mb-2">Parliamentary Starred / Executive Question Text</label>
            <textarea
              value={questionText}
              ...
              className="w-full bg-navy-950 border border-slate-700/80 rounded-xl px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-gold-500/80 focus:ring-1 focus:ring-gold-500/50"
            />
          </div>

          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2">
            <div className="flex items-center gap-3">
              <span className="text-xs font-mono text-slate-400 whitespace-nowrap">Intent Filter:</span>
              <select
                value={questionType}
                onChange={(e) => setQuestionType(e.target.value)}
                className="bg-navy-900 border border-slate-700/80 rounded-lg px-3 py-1.5 text-xs text-slate-200 font-mono focus:outline-none focus:border-gold-500"
              >
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-gold-500 to-amber-500 hover:from-gold-400 hover:to-amber-400 text-navy-950 font-bold text-sm shadow-glow-gold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
      ...
      {briefing && !loading && (
        <div className="space-y-6">
          ...
          <Card className="border-slate-800/90 shadow-card-dark bg-navy-950/90">
            <div className="p-6 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Landmark className="h-5 w-5 text-gold-400" />
                    <h2 className="text-lg font-bold text-slate-100">Parliamentary Briefing Note</h2>
                  </div>
                  <p className="text-xs text-slate-400 font-mono">
                    Target Scope: <span className="text-gold-400 font-bold">{briefing.selected_scope}</span> | Fiscal Year: <span className="text-emerald-400 font-bold">{briefing.fiscal_year}</span> | Compiled: {briefing.generated_at}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-navy-800 hover:bg-navy-700 border border-slate-700 text-slate-200 text-xs font-semibold flex items-center gap-2 transition-all shadow-sm"
                  >
              ...
              <div className="space-y-2">
                <h3 className="text-xs font-mono uppercase tracking-wider text-slate-400 font-bold">Executive Summary</h3>
                <p className="text-sm text-slate-200 leading-relaxed bg-navy-900/60 p-4 rounded-xl border border-slate-800">
              ...
              {briefing.key_findings.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-xs font-mono uppercase tracking-wider text-slate-400 font-bold">Key Findings & Operational Highlights</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {briefing.key_findings.map((finding, idx) => (
                      <div key={idx} className="p-3 bg-navy-900/40 border border-slate-800 rounded-xl flex items-start gap-2.5 text-xs text-slate-200">
              ...
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-800 bg-navy-950/40 text-[11px] font-mono text-slate-400 uppercase tracking-wider">
                    ...
                    <tbody className="divide-y divide-slate-800/60 text-xs font-mono">
                      {briefing.subsidiary_metrics.map((m, idx) => (
                        <tr key={idx} className="hover:bg-navy-800/40 transition-colors">
                          <td className="py-3 px-4 font-sans font-semibold text-slate-200">{m.mine_name}</td>
          ...
          {briefing.discrepancies.length > 0 && (
            <Card className="border-red-900/50 shadow-card-dark bg-red-950/10">
              <CardHeader className="py-3.5 px-4 bg-red-950/40 border-b border-red-900/50">
              ...
              <CardContent className="p-4 space-y-3">
                {briefing.discrepancies.map((d, idx) => (
                  <div key={idx} className="p-4 bg-navy-950 border border-red-900/40 rounded-xl space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-slate-100">{d.entity}</span>
                        <span className="text-xs text-slate-400 font-mono">({d.metric_name})</span>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono pt-1">
                      <div className="p-2.5 bg-navy-900 rounded-lg border border-slate-800">
                        <span className="text-slate-400 block text-[10px] truncate">{d.doc_a_filename}</span>
                        <span className="text-slate-100 font-bold text-sm block mt-0.5">{d.doc_a_value} {d.unit}</span>
                      </div>
                      <div className="p-2.5 bg-navy-900 rounded-lg border border-slate-800">
                        <span className="text-slate-400 block text-[10px] truncate">{d.doc_b_filename}</span>
                        <span className="text-slate-100 font-bold text-sm block mt-0.5">{d.doc_b_value} {d.unit}</span>
                      </div>
                      <div className="p-2.5 bg-red-950/60 rounded-lg border border-red-900/60 text-center">
                        <span className="text-red-300 block text-[10px]">Variance Percentage</span>
                        <span className="text-red-400 font-bold text-sm block mt-0.5">{d.variance_percentage}%</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <Card className="border-slate-800/90 shadow-card-dark">
              ...
              <CardContent className="p-4 space-y-3">
                {briefing.evidence.map((ev, idx) => (
                  <div key={idx} className="p-3.5 bg-navy-950 border border-slate-800 rounded-xl flex items-start justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2 text-xs font-mono">
                        <span className="text-gold-400 font-bold">{ev.document_name}</span>
                        <span className="text-slate-500">•</span>
                        <span className="text-slate-300">Page {ev.page_number}</span>
                        <span className="text-slate-500">•</span>
                        <span className="text-emerald-400">RRF Score: {ev.rrf_score}</span>
                      </div>
                      <p className="text-xs text-slate-300 line-clamp-2 leading-relaxed font-sans">{ev.text_snippet}</p>
                    </div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-navy-900 hover:bg-navy-800 border border-slate-700/80 text-gold-400 text-xs font-semibold shrink-0 transition-colors whitespace-nowrap"
                    >
                      View Evidence
                    </button>
                  </div>
          ...
          {briefing.limitations.length > 0 && (
            <Card className="border-slate-800/90 shadow-card-dark bg-navy-950/40">
              <div className="p-4 space-y-2">
                <h4 className="text-xs font-mono uppercase tracking-wider text-slate-400 font-bold">System Limitations & Scope Boundaries</h4>
